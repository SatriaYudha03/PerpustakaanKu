...
class ReturnFineSingleResource extends JsonResource
{
    ...
    public function toArray(Request $request): array
    {
        return [
            ...
            'loan' => $this->whenLoaded(relationship: 'loan', value: [
                'id' => $this->loan?->id,
                'loan_code' => $this->loan?->loan_code,
                'loan_date' => Carbon::parse(time: $this->loan?->loan_date)->format(format: 'd M Y'),
                'due_date' => Carbon::parse(time: $this->loan?->due_date)->format(format: 'd M Y'),
            ]),
