...
class BookController extends Controller
{
    ...
    public function index(): Response
    {
        ...
        return inertia('Admin/Books/Index', [
            'page_settings' => [
                'title' => 'Buku',
                'subtitle' => 'Menampilkan semua buku yang tersedia pada platform'
            ],
    ...
    public function create(): Response
    {
        return inertia('Admin/Books/Create', [
            ...
            'page_data'=>[
                'publicationYears' => range(2015, now()->year),
                'languages' => BookLanguage::option(),
    ...
    public function update(Book $book, BookRequest $request): RedirectResponse
    {
        try{
            $book->update([
                ...
                'number_of_pages'=>$request->number_of_pages,
                'status'=>$request->total > 0 ? BookStatus::AVAILABLE->value : BookStatus::UNAVAILABLE->value,
                'cover'=>$this->upload_file($request, $book, 'cover','books'),
                'price'=>$request->price,
                'category_id'=>$request->category_id,
                'publisher_id'=>$request->publisher_id,
            ]);
    ...
    private function bookCode(int $publication_year, int $category_id): string
    {
        ...
        return 'PU' . $publication_year . '.' . str()->slug($category->name) . '.' . $ordering;
    }
}
