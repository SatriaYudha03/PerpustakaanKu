...
class PublisherController extends Controller
{
    ...
    public function index(): Response
    {
        ...
        return inertia('Admin/Publishers/Index', [
            ...
            'publishers' => PublisherResource::collection($publisers)->additional([
                'meta' => [
                    'has_pages' => $publisers->hasPages(),
                ],
            ]),
            'state' => [
                'page' => request()->page ?? 1,
    ...
    public function store(PublisherRequest $request): RedirectResponse
    {
        try {
            Publisher::create([
                ...
                'email' => $request->email,
                'phone' => $request->phone,
                'logo' => $this->upload_file($request, 'logo', 'publisher'),
            ]);;
    ...
    public function update(Publisher $publisher, PublisherRequest $request) : RedirectResponse
    {
        try {
            $publisher->update([
                ...
                'email' => $request->email,
                'phone' => $request->phone,
                'logo' => $this->update_file($request, $publisher, 'logo', 'publisher'),
            ]);
